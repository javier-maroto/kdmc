...
for modulation_idx = 1:length(modulationTypes)
    ...
    if isfile(sprintf("../../data/signal/%s/%s_%s_%0.1g_%0.1g_%0.2g_%d.mat", "rx_x", channel, ...
            modulation, fs, sps, RC, snr))
        %continue
    end
    fprintf("%s - Generating %s_%s_%0.1g_%0.1g_%0.2g_%d.mat \n", ...
    ...
    for i = 1:n_signals
        ...
        xtm = mod(xts);
        xtf = tx_filt(xtm);
        xrf = applyChannel(xtf, channel, snr, fs);
        if(applyClockOffset==true)
            xrf = addClockOffset(xrf,fs,fc);
        end
        
        %xrs = demod(xrm);
        % Create frame
        if(applyRandomFrame==true)
            startIdx = randi([transDelay length(xrf)-transDelay-spf]);
        else
            startIdx = Ls*ceil(transDelay/Ls);
        end
        ...
        if ~contains(char(modulation), {'GFSK', 'CPFSK', 'B-FM', 'DSB-AM', 'SSB-AM','OQPSK'})
            ...
            rx_s(i,:) = xrm(startIdx/Ls+(1:n_symb));

            xrnf = applyChannel(xtm, channel, snr, fs);
            rx_nf(i,:) = xrnf(startIdx/Ls+(1:n_symb));
        else
            tx_s = -1;
            rx_s = -1;
            rx_nf = -1;
    ...
    for var = ["tx_s", "rx_x", "rx_s", "rx_nf", "y"]
